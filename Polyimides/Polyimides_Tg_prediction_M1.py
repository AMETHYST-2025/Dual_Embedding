...
embedding_model='PB_Tg_PI_Exp' #PB_Tg_PI_MM
dir = "./Polyimides/"
...
link_polyimides_embedding_set = dir + "Polyimides_synthetic_embedding.csv"
link = link_polyimides_embedding_set #
...
def train(DF_polyimides):
    ...
    for train_index, test_index in kf.split(X_polyBERT):
        ...
        #save the model to be fine-tuned:
        if k == 3: model.save(dir+"Model_M1/M1_MAE_polyimides_PB_Exp.keras")
        # -----------------------------------------------------------------------------
